...
exports.getYouTubeSubtitles = getYouTubeSubtitles;
...
async function getSubtitlesViaInnerTube(videoId) {
    ...
    const response = await (0, node_fetch_1.default)('https://www.youtube.com/youtubei/v1/player?key=AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8', {
        method: 'POST',
        headers: {
    ...
    console.log('[YouTube] Found caption tracks via InnerTube:', captionTracks.map((t) => ({
        lang: t.languageCode,
    ...
    const subtitleResponse = await (0, node_fetch_1.default)(subtitleUrl, {
        headers: {
 ...
 */
async function downloadSubtitlesFromBaseUrl(baseUrl) {
...
async function getYouTubeSubtitles(url) {
    ...
    try {
        ...
        console.log('[YouTube] InnerTube API failed:', error.message);
    }
    ...
    try {
        ...
        console.log('[YouTube] HTML page method failed:', error.message);
    }
    // All methods failed
