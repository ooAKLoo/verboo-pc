...
class BilibiliAdapter {
    ...
    getAuthorInfo() {
        ...
        for (const selector of nameSelectors) {
            upNameEl = document.querySelector(selector);
            if (upNameEl?.textContent?.trim())
                break;
        }
        if (!upNameEl)
            return null;
        ...
        for (const selector of avatarSelectors) {
            upAvatarEl = document.querySelector(selector);
            if (upAvatarEl?.src)
                break;
        }
        // UP主主页链接
        ...
        for (const selector of linkSelectors) {
            upLinkEl = document.querySelector(selector);
            if (upLinkEl?.href)
                break;
        }
        return {
...
// Auto-register
registry_1.adapterRegistry.register(new BilibiliAdapter());
